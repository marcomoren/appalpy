...
def get_tender_data(date):
    ...
    tender_csv_file = pd.read_csv(full_url, compression='zip', sep=';', warn_bad_lines=True, dtype='object')
...
def get_awards_data():
    ...
    awards_csv_file = pd.read_csv(awards_url, compression='zip', sep=';', warn_bad_lines=True, dtype='object')
...
def get_contractor_data():
    ...
    contractor_csv_file = pd.read_csv(contractor_url, compression='zip', sep=';', warn_bad_lines=True, dtype='object')
    return contractor_csv_file

def reduce_data(df, keywords, field):
    """
    ...
    if type(keywords) is list:
        ...
        raise ValueError("user input must be passed as a list or a string")
    return df